...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-6">
                    <ul class="breadcrumbs">
                        ...
                        <li>{{ trans('pricing_plans') }}</li>
                    </ul>
                    <h2 class="mb-2">{{ trans('pricing_plans') }}</h2>
                </div>
            </div>
        </div>
    </div><!--/vine-header-->
            ...
            @forelse($plans as $plan)
                <div class="col-lg-6 mb-5" data-aos="fade-up" data-aos-easing="linear">
                    <div class="price">
                        ...
                        @else
                            <a href="#sub-dialog" class="btn btn-mint btn-sm rounded-pill has-popup"><i class="bi bi-arrow-repeat me-1"></i>{{ trans('subscribe_now') }} / {{ get_setting('currency_symbol') }}{{ $plan->price }}</a>
                        @endif
                    </div>
                </div>
            @empty
    ...
    @else
        <div id="sub-dialog" class="white-popup zoom-anim-dialog mfp-hide">
                <h4>{{ trans('please_login_to_purchase_plans') }}.</h4>
        </div>
    @endif
