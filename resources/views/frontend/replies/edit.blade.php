...
@section('content')
            ...
            <div class="row px-3">
                <div class="col-lg-12">
                    <ul class="breadcrumbs">
                        ...
                        <li><a href="{{ route('home.post', ['post_id' => $reply->comment->post->post_id, 'slug' => $reply->comment->post->slug]) }}">{{ trans('back_to_post') }}</a></li>
                        <li>{{ trans('edit') }} {{ trans('reply') }}</li>
                    </ul>
                    <h2 class="mb-2">{{ trans('edit') }} {{ trans('reply') }}</h2>
                </div>
            </div>
        </div>
    </div><!--/vine-header-->
    ...
    <div class="comment-form" data-aos="fade-up" data-aos-easing="linear">
        <h3 class="comment-form-title">{{ trans('edit') }} {{ trans('reply') }}</h3>
                ...
                <div class="comment-form-avatar d-none d-sm-block">
                 ...
                 <div class="comment-input">
                    ...
                    <div class="comment-btn mt-3">
                        <button type="submit" class="btn btn-mint" id="edit_reply_btn">{{ trans('edit') }} {{ trans('reply') }}</button>
                    </div>
...
<script>
    ...
    $(function() {
     ...
     $(document).on('submit', '#edit_reply_form', function(e) {
       ...
       $("#edit_reply_btn").text('{{ trans('posting') }} {{ trans('reply') }}...');
       ...
       $.ajax({
         url: '{{ route('user.replies.edit', ['id' => $reply->id]) }}',
         method: 'post',
         data: fd,
         cache: false,
         contentType: false,
         ...
         success: function(response) {
           ...
           if (response.status == 400) {
               ...
               $("#edit_reply_btn").text('{{ trans('edit') }} {{ trans('reply') }}');
